# [KubeSphere 安装](https://kubesphere.io/zh/)
>KubeSphere 容器平台,面向云原生应用的容器混合云
...
#所有机器配置master域名
echo "192.168.10.122 k8s-master" >> /etc/hosts
```
#### 初始化master节点
1、初始化
```bash
...
and service account keys on each node and then running the following as root:
  ...
  kubeadm join k8s-master:6443 --token m79eud.o3wubng2xugjq746 \
    --discovery-token-ca-cert-hash sha256:dc0bd3f0565d6aacde382a107505b7a3e49dbd57f783d8529e3310a6e0bbad9f \
    --control-plane 
      ...
      containers:
        - name: nfs-client-provisioner
          ...
          env:
            ...
            - name: NFS_SERVER
              value: 192.168.10.102  ## 指定自己nfs服务器地址
            - name: NFS_PATH  
...
参照[官网](https://kubesphere.io/zh/docs/v3.4/pluggable-components/app-store/)“启用可插拔组件” 
### 执行安装
```bash
...
spec:
  ...
  etcd:
    ...
    endpointIps: 192.168.10.102   # etcd cluster EndpointIps. It can be a bunch of IPs here.
    port: 2379              # etcd port.
    tlsEnable: true
  common:
    redis:
...
# 检查是否以 root 用户运行脚本
if [[ $EUID -ne 0 ]]; then
   echo "此脚本必须以 root 用户或使用 sudo 执行" 
   exit 1
fi
...
# 配置 Docker 的 yum 源
echo "配置 Docker 的 yum 源..."
yum-config-manager --add-repo http://mirrors.aliyun.com/docker-ce/linux/centos/docker-ce.repo
if [[ $? -ne 0 ]]; then
    echo "配置 Docker 的 yum 源失败"
    exit 1
fi
