...
2. 点击 `新建文档`。
3. 在写作页选择分类目录、文件名。
4. 在 Vditor 编辑器中写 Markdown，默认使用类似 Typora 的即时渲染模式。
5. 需要检查排版时，可以用工具栏切换分屏预览、预览或全屏模式。
6. 点击 `GitHub 登录` 完成授权。
7. 点击 `提交`。
...
- 默认模式是 `ir` 即时渲染，写 Markdown 时会像 Typora 一样直接显示排版效果。
- 工具栏支持标题、加粗、斜体、删除线、引用、无序列表、有序列表、任务列表、缩进、代码块、行内代码、链接、表格、图片上传、撤销、重做、编辑模式切换、分屏预览、预览、全屏和大纲。
- 上传图片会继续走 `/api/github/asset`，图片写入仓库静态资源目录后再把 Markdown 图片语法插入当前光标位置。
- `Ctrl + S` 或 `Command + S` 会触发提交，提交成功后自动返回文档阅读页。
